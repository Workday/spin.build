...
/**
 * Test plugin with a {@link @PreProcess} task. Used to verify that pre-processors
 * are executed before the main task.
 */
public class PreProcessTestPlugin implements Plugin {

    /**
     * Set to {@code true} by {@link PreProcessor} when it runs.
     */
    public static final AtomicBoolean PRE_PROCESSOR_RAN = new AtomicBoolean(false);
    ...
    public static class MetaClass implements Plugin.MetaClass {
        ...
        public boolean isDetectedIn(final Path path) {
            return Files.exists(path.resolve("preprocess.marker"));
        }
    }
}
