...
6. Print workspace tree + available task names.
...
| Class | Purpose |
|---|---|
| `ModuleDescriptor` | Spin's own JPMS module descriptor; parsed from `module-info.java` or synthesized; `walk()` iterates requires graph |
| `ModuleReference` | Lightweight `(name, Optional<Version>)` identifier |
| `Artifact` | Maven four-tuple `(groupId, artifactId, version, type)`; semantic version comparison |
| `Artifact.Version` | Maven-compatible semantic version with `Comparable`; handles `ga`, `cr`, `a`, `b`, `m` aliases |
| `Artifact.Version.Range` | Version range predicate: soft `1.0`, hard `[1.0]`, half-open `[1.0,)`, union `(,1.1),(1.1,)` |
| `Artifact.Constraint` | Pairs Maven coordinates with a `Version.Range`; catalog entries |
...
| `ModuleVersioning` / `ProjectModuleVersioning` | `Resource` that provides `ModuleDescriptor.Version` from `version.properties` (glob-keyed) |
...
```
JavaPlugin (interface)           — @Provides JDKVersion, JDK, ModuleDescriptor
...
**`Java8CompilerPlugin` tasks:**
`DetectSourcePaths`, `DetectSourceFiles`, `DetectCompilationClassPath`, `Compile` (`@Category("compile","build")`), `JavaDoc` (`@Category("document","build")`, `@After(Compile)`)
...
| `AbstractDetectCompilationClassPath` | Resolves `requires` from `ModuleDescriptor`: in-workspace → compiled output dir; external → `ModuleCatalog` + `Artifact.Resolver`. |
| `AbstractCompile` | Invokes `javac @argfile`; uses `ModuleGraphClassifier.classify` to split classpath. Non-default-version output → `META-INF/versions/{N}/`. Parses verbose javac stderr into telemetry events. |
...
| `AbstractJavaLinker` | Invokes `jlink` then classifies output; uses `Configuration.empty()` (not boot) to avoid collisions with spin's own boot-layer modules. Renders launch script via Freemarker `script.ftl`. |

**Other plugin classes:**
- `ResourcePlugin` — copies `src/main/resources/` before compilation; `@Before(JavaCompilerPlugin.Compile)`.
...
**`script.ftl`** generates a `bin/<name>.sh` that sets `$MP=../modules`, `$LIB=../classpath`, and invokes `java --module-path $MP [-cp $classpath] -m <rootModule>/build.spin.application.Spin $@`.
...
**POM generation:** Loads from project root `pom.xml` or bundled template. Prunes to metadata whitelist. Regenerates `<dependencies>` from `ModuleDescriptor.requires()`.
...
### `spin-junit-module` — JUnit 5 Test Runner

**JPMS module:** `build.spin.module.junit`
**Purpose:** Compile test sources and run them via JUnit Platform Console Launcher.
...
`DetectSourcePaths`, `DetectSourceFiles`, `DetectCompilationClassPath` (includes main compile output), `Compile` (chains on main `Compile`), test execution via `AbstractTest.test(ClassPath, Path)`.

**`AbstractTest`:** Resolves `org.junit.platform:junit-platform-console-standalone` via `Artifact.Resolver`, launches `ConsoleLauncher --scan-classpath --reports-dir=<buildPath>/reports/tests`. Non-zero exit → `RuntimeException`.

**`ResourcePlugin`:** Copies `src/test/resources/` before compilation; activated only when a `JUnitPlugin` is already present AND `src/test/resources` exists.
...
Integration tests for the Java build plugin. Uses `@WorkspacePath` fixtures (`java-8`, `java-25`, `multi-release`, `broken-25`, `jdeps`).
...
- **Version management:** versions live in `pom.xml` properties, `version.properties` (glob-keyed), and `module-catalog.properties`; `module-info.java` `requires` statements must also be updated when dependencies change.
- **No Lombok**, no codegen frameworks.
